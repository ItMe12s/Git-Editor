...
namespace git_editor {
    ...
    namespace {
        ...
        std::optional<LevelState> reconstructRoot(
            CommitStore& store, GitService& svc, LevelKey const& levelKey
        ) {
            auto rows = store.list(levelKey);
            if (rows.empty()) return LevelState{};
            auto it = std::find_if(rows.begin(), rows.end(), [](CommitRow const& r) {
                return !r.parent.has_value();
            });
    ...
    Result<CommitId> GitService::commit(
        LevelKey const& levelKey, std::string const& message, std::string const& liveLevelStr
    ) {
        PhaseTimer total;
        PhaseTimer phase;
        ...
        if (parent) {
            ...
            if (!headState)
                return logAndFail<CommitId>("failed to reconstruct HEAD, refusing to commit");
        }
        ...
        if (delta.adds.empty() && delta.removes.empty() && delta.modifies.empty() &&
            delta.headerChanges.empty()) {
        ...
        geode::log::info(
            ...
            "diff={:.0f}ms mod={} adds={} rem={} dump={:.0f}ms blob={}B insert={:.0f}ms "
            "total={:.0f}ms",
            reconMs,
            parseMs,
            uuidMs,
            uuidStats.fingerprintHits,
            uuidStats.spatialFallbacks,
            uuidStats.freshUuids,
            diffMs,
            delta.modifies.size(),
            delta.adds.size(),
            delta.removes.size(),
    ...
    Prepared<LevelState> GitService::prepareCheckout(LevelKey const& levelKey, CommitId target) {
        ...
        std::string msg =
            "Checkout: " + (targetRow ? shorten(targetRow->message, 40) : std::to_string(target));
    ...
    Result<CommitId> GitService::finalizeCheckout(
        PendingHeadUpdate const& pending, LevelState const& applied
    ) {
    ...
    Prepared<RevertPayload> GitService::prepareRevert(LevelKey const& levelKey, CommitId target) {
        ...
        if (!targetRow->parent) {
            out.result =
                failResult<RevertPayload>("can't revert the initial commit (it has no parent)");
            return out;
        }
    ...
    Prepared<LevelState> GitService::prepareSquash(
        LevelKey const& levelKey, std::vector<CommitId> const& idsOldestFirst, std::string const& message
    ) {
        Prepared<LevelState> out;
        ...
        for (auto id : idsOldestFirst) {
            ...
            if (row->levelKey != levelKey) {
                out.result = failResult<LevelState>(
                    "Commit " + std::to_string(id) + " belongs to a different level"
                );
                return out;
            }
        ...
        if (parentOfOldest) {
            ...
            if (!recon) {
                out.result = failResult<LevelState>("reconstruct base failed");
                return out;
            }
        ...
        if (!target) {
            out.result = failResult<LevelState>("reconstruct target failed");
            return out;
        }
    ...
    Result<CommitId> GitService::finalizeSquash(PendingSquash const& pending, LevelState const& applied) {
        auto newId = m_store.squash(
            pending.levelKey,
            pending.idsOldestFirst,
            pending.parentOfOldest,
            pending.message,
            pending.deltaBlob
        );
    ...
    Prepared<LevelState> GitService::prepareImportLevelFrom(LevelKey const& dest, LevelKey const& src) {
        Prepared<LevelState> out;
        if (dest == src) {
    ...
    Result<void> GitService::finalizeImportLevelFrom(
        PendingHistoryReplace const& pending, LevelState const& applied
    ) {
    ...
    Result<void> GitService::exportLevelToGdge(
        LevelKey const& levelKey, std::filesystem::path const& outPath
    ) {
        ...
        auto const rootRow = std::find_if(rows.begin(), rows.end(), [](CommitRow const& row) {
            return !row.parent;
        });
    ...
    ImportPlan GitService::planImport(
        LevelKey const& dest, std::vector<std::filesystem::path> const& inPaths
    ) {
    ...
    Result<void> GitService::finalizeImportManyFromGdge(PendingMergeImport const& pending) {
        ...
        for (std::size_t i = 0; i < pending.commits.size(); ++i) {
            ...
            minted.push_back(*id);
            if (p.cacheState) {
                this->cachePut(*id, std::make_shared<LevelState const>(*p.cacheState));
            }
        }
        return {};
    }
    ...
    std::shared_ptr<LevelState const> GitService::reconstruct(CommitId commitId) {
        ...
        LevelStatePtr currentState = std::make_shared<LevelState const>();
        ...
        for (auto it = chain.rbegin(); it != chain.rend(); ++it) {
            ...
            currentState =
                std::make_shared<LevelState const>(apply(LevelState(*currentState), *delta, nullptr));
    ...
    void GitService::cachePut(CommitId id, LevelState state) {
        this->cachePut(id, std::make_shared<LevelState const>(std::move(state)));
    }
