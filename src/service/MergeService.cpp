...
namespace git_editor {
    ...
    std::optional<LevelState> mergeStates3Way(
        LevelState const& base, LevelState const& ours, LevelState const& theirs, int& conflictCount
    ) {
        conflictCount = 0;
        LevelState merged = ours;

        auto mergeFieldMap =
            [&](FieldMap& target, FieldMap const& b, FieldMap const& o, FieldMap const& t) {
                std::set<std::string> keys;
                for (auto const& [k, _] : b)
                    keys.insert(k);
                for (auto const& [k, _] : o)
                    keys.insert(k);
        ...
        for (auto const& [id, _] : base.objects)
            ids.insert(id);
        for (auto const& [id, _] : ours.objects)
            ids.insert(id);
        for (auto const& [id, _] : theirs.objects)
            ids.insert(id);
