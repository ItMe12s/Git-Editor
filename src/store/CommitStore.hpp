...
namespace git_editor {
    ...
    public:
        ...
        std::filesystem::path const& dbPath() const {
            return m_dbPath;
        }

        // deltaBlob: uncompressed delta JSON, stored compressed in SQLite.
        Result<CommitId> insertAndSetHead(
            LevelKey const& levelKey, std::optional<CommitId> parent,
            std::optional<CommitId> reverts, std::string const& message, std::string const& deltaBlob
        );
        ...
        std::optional<CommitId> squash(
            ...
            std::optional<CommitId> parentOfOldest, std::string const& message,
