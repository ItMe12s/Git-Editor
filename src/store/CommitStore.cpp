...
bool CommitStore::replaceLevelHistoryFromImport(
    LevelKey const& dest,
    std::vector<ImportedCommit> const& commits,
    std::optional<std::int64_t> importHeadIndex
) {
    if (!m_db) return false;
    auto const canonicalDest = this->resolveOrCreateCanonicalKey(dest);

    std::unordered_map<std::int64_t, ImportedCommit const*> byIndex;
    byIndex.reserve(commits.size());
    for (auto const& c : commits) {
        if (byIndex.contains(c.importIndex)) {
            geode::log::error("replaceLevelHistoryFromImport: duplicate import index {}", c.importIndex);
            return false;
        }
        byIndex[c.importIndex] = &c;
    }

    for (auto const& c : commits) {
        if (c.parentIndex && !byIndex.contains(*c.parentIndex)) {
            geode::log::error("replaceLevelHistoryFromImport: parent index {} missing", *c.parentIndex);
            return false;
        }
        if (c.revertsIndex && !byIndex.contains(*c.revertsIndex)) {
            geode::log::error("replaceLevelHistoryFromImport: reverts index {} missing", *c.revertsIndex);
            return false;
        }
    }
    if (importHeadIndex && !byIndex.contains(*importHeadIndex)) {
        geode::log::error("replaceLevelHistoryFromImport: head index {} missing", *importHeadIndex);
        return false;
    }

    std::unordered_map<std::int64_t, int> inDeg;
    std::unordered_map<std::int64_t, std::vector<std::int64_t>> afterDone;
    for (auto const& [idx, rowPtr] : byIndex) {
        std::unordered_set<std::int64_t> deps;
        if (rowPtr->parentIndex) deps.insert(*rowPtr->parentIndex);
        if (rowPtr->revertsIndex) deps.insert(*rowPtr->revertsIndex);
        inDeg[idx] = static_cast<int>(deps.size());
        for (auto dep : deps) {
            afterDone[dep].push_back(idx);
        }
    }

    std::queue<std::int64_t> q;
    for (auto const& [idx, deg] : inDeg) {
        if (deg == 0) q.push(idx);
    }
    std::vector<std::int64_t> order;
    order.reserve(byIndex.size());
    while (!q.empty()) {
        auto u = q.front();
        q.pop();
        order.push_back(u);
        for (auto v : afterDone[u]) {
            inDeg[v]--;
            if (inDeg[v] == 0) q.push(v);
        }
    }
    if (order.size() != byIndex.size()) {
        geode::log::error("replaceLevelHistoryFromImport: topological sort failed");
        return false;
    }
    ...
    std::unordered_map<std::int64_t, CommitId> idMap;
    idMap.reserve(byIndex.size());
    for (auto idx : order) {
        auto const& src = *byIndex.at(idx);
        std::optional<CommitId> parent;
        std::optional<CommitId> reverts;
        if (src.parentIndex) parent = idMap.at(*src.parentIndex);
        if (src.revertsIndex) reverts = idMap.at(*src.revertsIndex);
        auto const newId = this->insertAt(
            canonicalDest, parent, reverts, src.message, src.createdAt, src.deltaBlob
        );
        if (!newId) {
            geode::log::error("replaceLevelHistoryFromImport: insert failed at {}", idx);
            execOrLog(m_db, "ROLLBACK;");
            execOrLog(m_db, "PRAGMA defer_foreign_keys = OFF;");
            return false;
        }
    ...
    if (importHeadIndex) {
        if (!this->setHead(canonicalDest, idMap.at(*importHeadIndex))) {
            geode::log::error("replaceLevelHistoryFromImport: setHead failed");
            execOrLog(m_db, "ROLLBACK;");
            execOrLog(m_db, "PRAGMA defer_foreign_keys = OFF;");
            return false;
        }
