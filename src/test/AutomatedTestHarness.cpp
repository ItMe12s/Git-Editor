...
namespace git_editor {
    ...
    void ReportBuilder::addPass(
        std::string const& suite, std::string const& name, std::string detail, double ms
    ) {
        out.rows.push_back({suite, name, "PASS", std::move(detail), ms});
        ++out.passCount;
    }

    void ReportBuilder::addFail(
        std::string const& suite, std::string const& name, std::string detail, double ms
    ) {
