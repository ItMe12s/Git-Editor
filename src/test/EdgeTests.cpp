...
namespace git_editor {
    ...
    void runEdgeTests(
        ...
        if (!st.dbPath().empty()) {
            ...
            if (!std::filesystem::exists(p)) {
                R.addFail(
                    kSuiteEdge,
                    "db_exists",
                    fmt::format("missing {}", geode::utils::string::pathToString(p)),
                    dbCheck.ms()
                );
            }
            else {
                ...
                R.addPass(
                    kSuiteEdge, "db_under_save", geode::utils::string::pathToString(p), dbCheck.ms()
                );
            }
        }
        else {
        ...
        if (!requireCommit(
                git, R, kSuiteEdge, "unicode_setup", unicodeRoundtrip.ms(), kZipEx, "unicode_path", levelAt(7)
            )) {
            return;
        }
        std::filesystem::path unicodeFile =
            testDir / std::filesystem::path(std::u8string(u8"at_\u0442\u0435\u0441\u0442.gdge"));
        R.addAction(
            kSuiteEdge,
            fmt::format("unicode export {}", geode::utils::string::pathToString(unicodeFile))
        );
        if (auto ex = git.exportLevelToGdge(kZipEx, unicodeFile); !ex) {
            ...
            if (!imp) {
                ...
                R.addPass(
                    kSuiteEdge,
                    "unicode_roundtrip",
                    geode::utils::string::pathToString(unicodeFile),
                    unicodeRoundtrip.ms()
                );
            }
        }
        ...
        if (foundHistDst) {
            R.addFail(
                kSuiteEdge,
                "list_levels_after_delete",
                "deleted key still in listLevels",
                listLevelsT.ms()
            );
            return;
        }
        R.addPass(
            kSuiteEdge, "list_levels_after_delete", "kHistDst absent from listLevels", listLevelsT.ms()
        );
        ...
        auto largeInsert =
            st.insertAndSetHead(kHistDst, std::nullopt, std::nullopt, "large_delta", largeDelta);
        if (!largeInsert) {
        ...
        if (!foundRenamed) {
            R.addFail(
                kSuiteEdge,
                "update_message_verify",
                "listSummaries missing renamed message",
                updateMsgT.ms()
            );
            return;
        }
        ...
        if (!requireCommit(
                git, R, kSuiteEdge, "plan_setup_commit", planT.ms(), kRawEx, "plan_probe", levelAt(3)
            )) {
            return;
        }
        ...
        if (plan.smart.empty() && plan.sequential.empty()) {
            R.addFail(
                kSuiteEdge, "plan_buckets", "expected file in smart or sequential bucket", planT.ms()
            );
            return;
        }
        ...
        if (validCount != 1) {
            ...
            R.addFail(
                ...
                "mixed_bag_path",
                fmt::format(
                    "invalid bucket holds wrong path: {}",
                    geode::utils::string::pathToString(mixedPlan.invalid[0].path)
                ),
                mixedBagT.ms()
            );
        }
        else if (mixedPlan.invalid[0].reason.empty()) {
            R.addFail(kSuiteEdge, "mixed_bag_reason", "garbage entry reason empty", mixedBagT.ms());
        }
        else {
            R.addPass(
                kSuiteEdge,
        ...
        if (!foundStartPos) {
            R.addFail(
                kSuiteEdge,
                "startpos_roundtrip",
                "startpos object missing after parse",
                startposRound.ms()
            );
        }
        else if (!kAPreserved) {
            R.addFail(
                kSuiteEdge, "startpos_roundtrip", "kA* keys lost during parse", startposRound.ms()
            );
        }
        else {
            ...
            if (!match) {
                R.addFail(
                    kSuiteEdge,
                    "startpos_roundtrip",
                    "fields drift across parse->serialize->parse",
                    startposRound.ms()
                );
            }
            else {
