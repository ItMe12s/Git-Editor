...
namespace git_editor {
    ...
    void runAdvancedCollabSimulatorTests(
        GitService& git, CommitStore& st, std::filesystem::path const& testDir, ReportBuilder& R
    ) {
        ScopedTimer T;
        ...
        if (!requireCommit(
                git, R, kSuiteAdvancedCollab, "base_commit", T.ms(), kAdvCollabBase, "adv_base", levelAt(0)
            )) {
            return;
        }
        if (!requireExport(
        ...
        if (!forkExport(
                git, R, kSuiteAdvancedCollab, T.ms(), kAdvCollabBob, basePath, bobPath, 302, "bob"
            )) {
            return;
        }
        if (!forkExport(
        ...
        R.addPass(
            ...
            fmt::format(
                "triple smart {} mix smart {} seq {}",
                planTriple.smart.size(),
                planMix.smart.size(),
                planMix.sequential.size()
            ),
            T.ms()
        );
        ...
        auto batch =
            git.importManyFromGdge(kAdvCollabIntegrator, {bogusPath, alicePath, bobPath, scratchPath});
        if (!batch) {
        ...
        if (batch->skippedCount < 1) {
            R.addFail(
                kSuiteAdvancedCollab,
                "invalid_skip_count",
                fmt::format("skipped {}", batch->skippedCount),
                T.ms()
            );
            return;
        }
        if (batch->smartCount != 3) {
            R.addFail(
                kSuiteAdvancedCollab,
                "triple_merge_shape",
                fmt::format("smart {}", batch->smartCount),
                T.ms()
            );
            return;
        }
        ...
        R.addPass(
            ...
            fmt::format(
                "merged {} smart {} conflicts {}", batch->mergedCount, batch->smartCount, batch->conflictCount
            ),
            T.ms()
        );
        ...
        if (!requireImportMany(
                git, R, kSuiteAdvancedCollab, "integrator_reset_import", T.ms(), kAdvCollabIntegrator, {basePath}
            )) {
            return;
        }
        if (!requireCommit(
                ...
                R,
                kSuiteAdvancedCollab,
                "integrator_pre_cf_commit",
                T.ms(),
                kAdvCollabIntegrator,
                "integrator_pre_conflict",
                levelAt(10)
            )) {
            return;
        }
        ...
        if (!requireImportMany(
                git, R, kSuiteAdvancedCollab, "cf_bob_import", T.ms(), kAdvCollabBob, {basePath}
            )) {
            return;
        }
        if (!requireCommit(
                git, R, kSuiteAdvancedCollab, "bob_cf_commit", T.ms(), kAdvCollabBob, "bob_cf", conflictB
            )) {
            return;
        }
        if (!requireExport(
                git, R, kSuiteAdvancedCollab, "export_cf_bob", T.ms(), kAdvCollabBob, conflictBobExport
            )) {
            return;
        }
        ...
        if (!requireImportMany(
                git, R, kSuiteAdvancedCollab, "cf_cara_import", T.ms(), kAdvCollabCara, {basePath}
            )) {
            return;
        }
        if (!requireCommit(
                git, R, kSuiteAdvancedCollab, "cara_cf_commit", T.ms(), kAdvCollabCara, "cara_cf", conflictC
            )) {
            return;
        }
        if (!requireExport(
                git, R, kSuiteAdvancedCollab, "export_cf_cara", T.ms(), kAdvCollabCara, conflictCaraExport
            )) {
            return;
        }
        ...
        auto mergeCf =
            git.importManyFromGdge(kAdvCollabIntegrator, {conflictBobExport, conflictCaraExport});
        if (!mergeCf) {
        ...
        if (mergeCf->conflictCount <= 0) {
            R.addFail(
                kSuiteAdvancedCollab,
                "merge_cf_count",
                fmt::format("conflictCount {}", mergeCf->conflictCount),
                T.ms()
            );
            return;
        }
        ...
        if (chain.size() < 2) {
            R.addFail(
                kSuiteAdvancedCollab, "final_chain", fmt::format("commits {}", chain.size()), T.ms()
            );
            return;
        }
        R.addPass(
