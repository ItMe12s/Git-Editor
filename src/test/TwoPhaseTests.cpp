    ...
    void runTwoPhaseTests(
        ...
        if (!headMid || *headMid != *headBefore || rowsMid != rowsBefore) {
            R.addFail(
                ...
                "prepare_no_db_write",
                fmt::format(
                    "head {}->{}, rows {}->{}", *headBefore, headMid ? *headMid : -1, rowsBefore, rowsMid
                ),
                checkoutPhase.ms()
            );
            return;
        ...
        if (!headAfter || *headAfter == *headBefore) {
            R.addFail(
                kSuiteTwoPhase, "finalize_moves_head", "HEAD did not advance", checkoutPhase.ms()
            );
            return;
        }
        if (st.list(kTwoPhase).size() != rowsBefore + 1) {
            R.addFail(
                kSuiteTwoPhase, "finalize_row_count", "expected one new commit row", checkoutPhase.ms()
            );
            return;
        }
        ...
        if (!applied || !c1State || hashLevelState(*applied) != hashLevelState(*c1State)) {
            R.addFail(
                kSuiteTwoPhase, "finalize_state", "reconstructed state != target c1", checkoutPhase.ms()
            );
            return;
        }
        R.addPass(
            kSuiteTwoPhase, "checkout_two_phase", "prepare/finalize split OK", checkoutPhase.ms()
        );
        ...
        if (prepNoop.pendingHead) {
            R.addFail(
                kSuiteTwoPhase,
                "prepare_noop_pending",
                "expected no pendingHead when HEAD==target",
                noopPhase.ms()
            );
            return;
        }
        ...
        R.addAction(
            kSuiteTwoPhase,
            fmt::format("export for import test {}", geode::utils::string::pathToString(gdgePath))
        );
        st.deleteLevel(kRawEx);
        if (!requireCommit(
                git, R, kSuiteTwoPhase, "export_setup", suiteT.ms(), kRawEx, "tp_export", levelAt(5)
            )) {
            return;
        }
        ...
        if (!prepImp.pendingMergeImport) {
            R.addFail(
                kSuiteTwoPhase, "prepare_import_pending", "expected pendingMergeImport", importPhase.ms()
            );
            return;
        }
        if (st.getHead(kMix).has_value() || !st.list(kMix).empty()) {
            R.addFail(
                kSuiteTwoPhase,
                "prepare_import_no_write",
                "dest DB mutated before finalize",
                importPhase.ms()
            );
            return;
        }
        ...
        if (!headMix) {
            R.addFail(
                kSuiteTwoPhase, "finalize_import_head", "no HEAD after finalize", importPhase.ms()
            );
            return;
        }
        ...
        if (hashLevelState(*mixRecon) != hashLevelState(prepImp.result->state)) {
            R.addFail(
                kSuiteTwoPhase,
                "finalize_import_state",
                "DB state != prepared payload state",
                importPhase.ms()
            );
            return;
        }
        R.addPass(
            kSuiteTwoPhase,
            "import_two_phase",
            "prepare/finalize import on empty dest OK",
            importPhase.ms()
        );
    }
