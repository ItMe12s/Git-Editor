    ...
    void runGdgeExportImportTests(
        ...
        if (!guard.ok()) {
            ...
            R.addSkip(
                kSuiteGdge,
                "compress_toggle",
                "ModSettingsManager or bool setting unavailable",
                compressSkip.ms()
            );
            return;
        }
        ...
        if (!requireCommit(
                git, R, kSuiteGdge, "setup_export_levels", setupLevels.ms(), kRawEx, "one", levelAt(1)
            ) ||
            !requireCommit(
                git, R, kSuiteGdge, "setup_export_levels", setupLevels.ms(), kZipEx, "one", levelAt(2)
            )) {
            return;
        }
        ...
        R.addAction(
            kSuiteGdge,
            fmt::format("exportLevelToGdge raw {}", geode::utils::string::pathToString(rawPath))
        );
        ...
        R.addAction(
            kSuiteGdge,
            fmt::format("raw first bytes len {} sqlite={}", rawSig.size(), startsWithSqlite(rawSig))
        );
        ...
        R.addAction(
            kSuiteGdge,
            fmt::format("exportLevelToGdge zip {}", geode::utils::string::pathToString(zipPath))
        );
        ...
        if (payloadHash != dbHash) {
            R.addFail(
                kSuiteGdge, "import_state_hash", "payload state != reconstructed HEAD", importPhase.ms()
            );
            return;
        }
        ...
        if (hashAgain != dbHash) {
            R.addFail(
                kSuiteGdge, "import_state_stable", "hash changed on second reconstruct", importPhase.ms()
            );
            return;
        }
