...
namespace git_editor {
    ...
    void runCollabPlanTest(
        ...
        if (!requireExport(
                ...
                kOther,
                otherPath,
                fmt::format("export other {}", geode::utils::string::pathToString(otherPath))
            )) {
            return;
        }
        ...
        if (plan.smart.size() != 2) {
            R.addFail(
                kSuiteCollab, "smart_pair", fmt::format("smart size {}", plan.smart.size()), T.ms()
            );
            return;
        }
        ...
        R.addAction(
            kSuiteCollab,
            fmt::format("plan A+Other smart {} seq {}", planMix.smart.size(), planMix.sequential.size())
        );
        ...
        if (mergeTwo->smartCount < 2) {
            R.addFail(
                kSuiteCollab,
                "merge_smart_count",
                fmt::format("smartCount {}", mergeTwo->smartCount),
                T.ms()
            );
            return;
        }
        ...
        if (!requireImportMany(
                ...
                kDecA,
                {basePath},
                "importMany kDecA <- base conflict path"
            )) {
            return;
        }
        if (!requireCommit(
        ...
        if (!requireExport(
                ...
                kDecA,
                decAPath,
                fmt::format("export a_conflict {}", geode::utils::string::pathToString(decAPath))
            )) {
            return;
        }
        if (!requireImportMany(
                ...
                kDecB,
                {basePath},
                "importMany kDecB <- base conflict path"
            )) {
            return;
        }
        if (!requireCommit(
        ...
        if (!requireExport(
                ...
                kDecB,
                decBPath,
                fmt::format("export b_conflict {}", geode::utils::string::pathToString(decBPath))
            )) {
            return;
        }
        ...
        if (mergeConflict->conflictCount <= 0) {
            R.addFail(
                ...
                fmt::format("conflictCount {}", mergeConflict->conflictCount),
                T.ms()
            );
            return;
        }
        R.addAction(
            kSuiteCollab, fmt::format("mergeConflict conflictCount {}", mergeConflict->conflictCount)
        );
