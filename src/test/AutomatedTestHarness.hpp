...
namespace git_editor {
    ...
    std::optional<CommitId> requireCommit(
        GitService& git, ReportBuilder& R, std::string_view suite, std::string_view failName,
        double elapsedMs, LevelKey const& key, std::string_view message,
        std::string const& levelData, std::string_view actionText = {}
    );

    bool requireExport(
        GitService& git, ReportBuilder& R, std::string_view suite, std::string_view failName,
        double elapsedMs, LevelKey const& key, std::filesystem::path const& outPath,
        std::string_view actionText = {}
    );

    std::optional<ImportManyPayload> requireImportMany(
        GitService& git, ReportBuilder& R, std::string_view suite, std::string_view failName,
        double elapsedMs, LevelKey const& dest, std::vector<std::filesystem::path> const& paths,
        std::string_view actionText = {}
    );

    bool forkExport(
        GitService& git, ReportBuilder& R, std::string_view suite, double elapsedMs,
        LevelKey const& key, std::filesystem::path const& basePath,
        std::filesystem::path const& outPath, int xField, char const* label
    );

    void formatReport(
        AutomatedTestSummary& s, std::filesystem::path const& saveDir, std::string const& modId
    );
