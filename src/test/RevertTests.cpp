...
namespace git_editor {
    ...
    void runRevertTests(GitService& git, CommitStore& st, ReportBuilder& R) {
        ...
        if (!git.commit(kRevert, "r1", levelAt(0)) || !git.commit(kRevert, "r2", levelAt(10)) ||
        ...
        if (chain.size() != 4) {
            R.addFail(
                kSuiteRevert, "chain_len", fmt::format("expected 4 commits got {}", chain.size()), T.ms()
            );
            return;
        }
        ...
        R.addAction(
            kSuiteRevert, fmt::format("revert revertCommit ok conflicts {}", rev1->conflicts.size())
        );
        ...
        st.deleteLevel(kRevert);
        if (!git.commit(kRevert, "cf1", levelAt(0)) || !git.commit(kRevert, "cf2", levelAt(10))) {
        ...
        if (!git.commit(kRevert, "cf3", cf3Level) || !git.commit(kRevert, "cf4", levelAtFixedX(30))) {
        ...
        if (cfChain.size() != 4) {
            R.addFail(
                kSuiteRevert, "conflict_chain_len", fmt::format("got {}", cfChain.size()), conflictT.ms()
            );
            return;
        }
