    ...
    namespace {
        ...
        FieldMap fieldMapFromJson(matjson::Value const& v) {
            ...
            for (auto const& entry : v) {
                ...
                if (asStr.isOk())
                    out.emplace(std::string(key->data(), key->size()), asStr.unwrap());
            }
            return out;
        }
...
namespace matjson {
    ...
    struct Serialize<git_editor::Delta::Modify> {
        ...
        static geode::Result<git_editor::Delta::Modify> fromJson(Value const& value) {
            ...
            if (auto fields = value.get<std::map<std::string, git_editor::FieldChange>>("fields");
                fields.isOk()) {
    ...
    struct Serialize<git_editor::Delta> {
        ...
        static geode::Result<git_editor::Delta> fromJson(Value const& value) {
            ...
            if (auto changes = value.get<std::map<std::string, git_editor::FieldChange>>("h");
                changes.isOk()) {
            ...
            if (auto modifies = value.get<std::vector<git_editor::Delta::Modify>>("~");
                modifies.isOk()) {
