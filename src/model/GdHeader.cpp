...
namespace git_editor::gd_header {
    ...
    std::optional<std::string> mergeHeaders3Way(
        std::string_view base, std::string_view ours, std::string_view theirs, int& outConflicts
    ) {
        outConflicts = 0;
        ...
        for (auto const& [k, v] : *bParsed)
            bMap[k] = v;
        for (auto const& [k, v] : *oParsed)
            oMap[k] = v;
        for (auto const& [k, v] : *tParsed)
            tMap[k] = v;
        ...
        for (auto const& [k, _] : *oParsed)
            pushKey(k);
        for (auto const& [k, _] : *tParsed)
            pushKey(k);
        for (auto const& [k, _] : *bParsed)
            pushKey(k);

        KvList mergedKv;
        for (auto const& key : keyOrder) {
            std::optional<std::string> b =
                bMap.contains(key) ? std::optional<std::string>(bMap.at(key)) : std::nullopt;
            std::optional<std::string> o =
                oMap.contains(key) ? std::optional<std::string>(oMap.at(key)) : std::nullopt;
            std::optional<std::string> t =
                tMap.contains(key) ? std::optional<std::string>(tMap.at(key)) : std::nullopt;

            if (key == "kS38") {
                auto parseOpt = [](
                                    std::optional<std::string> const& v
                                ) -> std::optional<std::optional<ChannelMap>> {
                ...
                if (eq(*oCh, *tCh)) mergedChannels = *oCh;
                    ...
                    std::set<int> ids;
                    if (*bCh)
                        for (auto const& [id, _] : **bCh)
                            ids.insert(id);
                    if (*oCh)
                        for (auto const& [id, _] : **oCh)
                            ids.insert(id);
                    if (*tCh)
                        for (auto const& [id, _] : **tCh)
                            ids.insert(id);
                    ChannelMap outMap;
                    for (int id : ids) {
                        std::optional<ChannelRecord> bRec = (*bCh && (**bCh).contains(id)) ?
                            std::optional<ChannelRecord>((**bCh).at(id)) :
                            std::nullopt;
                        std::optional<ChannelRecord> oRec = (*oCh && (**oCh).contains(id)) ?
                            std::optional<ChannelRecord>((**oCh).at(id)) :
                            std::nullopt;
                        std::optional<ChannelRecord> tRec = (*tCh && (**tCh).contains(id)) ?
                            std::optional<ChannelRecord>((**tCh).at(id)) :
                            std::nullopt;
