...
namespace git_editor {
    ...
    LevelBrowserLayer* LevelBrowserLayer::create(LevelEditorLayer* editor, EditorPauseLayer* pauseLayer) {
    ...
    bool LevelBrowserLayer::init(LevelEditorLayer* editor, EditorPauseLayer* pauseLayer) {
        if (!editor) {
            return false;
        }
        m_editor = editor;
...

        {
            ...
            auto totalLbl =
                CCLabelBMFont::create(("Total: " + formatBytes(dbBytes)).c_str(), "chatFont.fnt");
            totalLbl->setID("git-editor-levels-total"_spr);
            totalLbl->setScale(.5f);
            totalLbl->setAnchorPoint({1.f, .5f});
            totalLbl->setOpacity(200);
            m_mainLayer->addChildAtPosition(totalLbl, Anchor::TopRight, {-12.f, -16.f});
        }

        m_scroll =
            scroll_list_popup::attachScrollList(this, m_mainLayer, "git-editor-levels-scroll"_spr);
    ...
    bool LevelBrowserLayer::closeOnce(CCObject* sender) {
        return scroll_list_popup::closeOnce(this, m_listState, sender, [this](CCObject* s) {
            onClose(s);
        });
    }
    ...
    void LevelBrowserLayer::renderList(std::vector<LevelSummary> levels) {
        ...
        auto* content = scroll_list_popup::beginListRender(m_scroll, "git-editor-levels-loading"_spr);
        if (!content) return;
        ...
        auto makeRowBtn =
            [](
                char const* label, char const* texture, geode::Function<void(CCMenuItemSpriteExtra*)> cb
        ...
        for (auto const& lv : levels) {
            ...
            auto loadBtn = makeRowBtn(
                "Load", "GJ_button_02.png", [this, levelKey, destKey, count](CCMenuItemSpriteExtra*) {
                    Ref<LevelBrowserLayer> self(this);
                    if (!tryBeginBusyAction(self->m_busy)) return;
                    if (levelKey == destKey) {
                        finishBusyAction(self->m_busy);
                        Notification::create(
                            "Already this level: nothing to load", NotificationIcon::Info
                        )
                            ->show();
                        return;
                    }
                    ...
                    std::string const warnBody =
                        "This will PERMANENTLY DELETE all objects and the commit history for your "
                        "current level, "
                        "and overwrite it with the commit history from the selected level.\n"
                        "Make sure you're on an EMPTY level before doing this.\nThis CANNOT be "
                        "undone.\n"
                        "Selected history: " +
                        std::to_string(count) + " commit(s) from \"" + shorten(levelKey, 32) + "\".";
                    ...
                    createQuickPopup(
                        ...
                        [self, levelKey, destKey](FLAlertLayer*, bool yes) {
                            ...
                            prepared_editor_flow::run<LevelState, PendingHistoryReplace, void>(
                                ...
                                [](Prepared<LevelState> const& prep) {
                                    return prep.pendingReplace;
                                },
                                ...
                                prepared_editor_flow::OutcomeHandlers{
                                    .onSuccess =
                                        [self, pauseRef]() {
                                            Notification::create(
                                                "Level loaded", NotificationIcon::Success
                                            )
                                                ->show();
                                    ...
                                    .onFinalizeError =
                                        [](std::string const& error) {
                                            Notification::create(
                                                ("Editor applied but history copy failed: " + error)
                                                    .c_str(),
                                                NotificationIcon::Error
                                            )
                                                ->show();
            ...
            auto deleteBtn = makeRowBtn(
                "Delete", "GJ_button_06.png", [this, levelKey, count](CCMenuItemSpriteExtra*) {
                    Ref<LevelBrowserLayer> self(this);
                    if (!tryBeginBusyAction(self->m_busy)) return;
                    createQuickPopup(
                        "ARE YOU SURE?",
                        ("PERMANENTLY DELETE all " + std::to_string(count) + " commits for \"" +
                        ...
                        [self, levelKey](FLAlertLayer*, bool yes) {
                            ...
                            ui_action_runner::runWorkerResult<bool>(
                                ...
                                [alive](bool ok) {
                                    ...
                                    if (!ok) {
                                        Notification::create("Delete failed", NotificationIcon::Error)
                                            ->show();
                                        return;
                                    }
                                    Notification::create(
                                        "Level history removed", NotificationIcon::Success
                                    )
                                        ->show();
