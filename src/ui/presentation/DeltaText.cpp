...
namespace git_editor {
    ...
    std::string describeDeltaText(Delta const& d) {
        ...
        if (d.headerChanges.empty() && !d.rawHeaderChange.has_value() && d.adds.empty() &&
            d.removes.empty() && d.modifies.empty()) {
        ...
        for (auto const& [k, ch] : d.headerChanges) {
            appendStr(
                "Header " + fieldKeyName(k) + ": " + shorten(ch.before, kMaxField) + " -> " +
                shorten(ch.after, kMaxField)
            );
        }
        if (d.rawHeaderChange.has_value()) {
            appendStr(
                "Header raw: " + shorten(d.rawHeaderChange->before, kMaxField) + " -> " +
        ...
        for (auto const& m : d.modifies) {
            ...
            for (auto const& [k, ch] : m.fields) {
                appendStr(
                    "  " + fieldKeyName(k) + ": " + shorten(ch.before, kMaxField) + " -> " +
                    shorten(ch.after, kMaxField)
                );
            }
        }
