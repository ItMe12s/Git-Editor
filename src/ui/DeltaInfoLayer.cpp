...
namespace git_editor {
    ...
    DeltaInfoLayer* DeltaInfoLayer::createAndLoad(
        std::string title, geode::Function<Result<std::string>(void)> loadFn
    ) {
        ...
        auto loadFnHolder =
            std::make_shared<geode::Function<Result<std::string>(void)>>(std::move(loadFn));
    ...
    bool DeltaInfoLayer::init(std::string title, std::string body) {
        ...
        auto* scrollBg =
            CCLayerColor::create({0, 0, 0, static_cast<GLubyte>(kPanelAlpha)}, m_scrollW, m_scrollH);
        ...
        auto makeNavBtn =
            [](char const* label, char const* tex, geode::Function<void(CCMenuItemSpriteExtra*)> cb) {
        ...
        wrapper->addChildAtPosition(
            m_btnMenu, Anchor::BottomLeft, {kPanelPad + 22.f, kPanelPad + kNavH * 0.5f}
        );
        ...
        wrapper->addChildAtPosition(
            m_pageLabel, Anchor::BottomRight, {-kPanelPad, kPanelPad + kNavH * 0.5f}
        );
