...
namespace git_editor::scroll_list_popup {
    ...
    namespace {
        ...
        void showScrollOverlay(
            alpha::ui::AdvancedScrollLayer* scroll, char const* text, char const* overlayId,
            float opacity
        ) {
    ...
    alpha::ui::AdvancedScrollLayer* attachScrollList(
        ...
        mainLayer->addChildAtPosition(scroll, Anchor::Center, {-w * .5f, -h * .55f}, false);
        return scroll;
    }
    ...
    void showCenteredLabel(
        alpha::ui::AdvancedScrollLayer* scroll, char const* text, char const* labelId, float opacity
    ) {
        if (!scroll) return;
