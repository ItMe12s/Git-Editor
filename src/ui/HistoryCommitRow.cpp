...
namespace git_editor::history_rows {
    ...
    CCNode* createCommitRow(
        CommitSummary const& c, float rowWidth, bool squashMode, bool selected, HistoryLayer* layer
    ) {
        ...
        auto timeLbl = CCLabelBMFont::create(formatTimestamp(c.createdAt).c_str(), "chatFont.fnt");
        timeLbl->setScale(.5f);
        ...
        auto msgLbl = CCLabelBMFont::create(shorten(c.message, 34).c_str(), "chatFont.fnt");
        msgLbl->setScale(.55f);
        ...
        menu->setLayout(
            RowLayout::create()->setGap(4.f)->setAxisAlignment(AxisAlignment::End)->setCrossAxisOverflow(true)
        );
        ...
        if (squashMode) {
            auto tickBtn = CCMenuItemExt::createTogglerWithStandardSprites(
                .6f, [layer, commitId](CCMenuItemToggler*) {
            ...
            auto helpBtn =
                makeBtn("?", "GJ_button_04.png", [layer, commitId, commitMsg](CCMenuItemSpriteExtra*) {
                    if (!layer || layer->m_listState.closing) return;
            ...
            auto renameBtn = makeBtn(
                "Rename", "GJ_button_04.png", [layer, commitId, commitMsg](CCMenuItemSpriteExtra*) {
                    if (!layer || layer->m_listState.closing) return;
                    ...
                    if (auto popup = CommitMessageLayer::create(
                            [self, commitId](std::string const& newMessage) {
                                ui_action_runner::runWorkerResult<bool>(
                                    [commitId, newMessage]() {
                                        return sharedGitService().updateCommitMessage(
                                            commitId, newMessage
                                        );
                                    },
                                    [self](bool ok) {
                                        ...
                                        if (!ok) {
                                            Notification::create("Rename failed", NotificationIcon::Error)
                                                ->show();
                                            return;
                                        }
                                        Notification::create("Renamed commit", NotificationIcon::Success)
                                            ->show();
            ...
            auto checkoutBtn = makeBtn(
                "Checkout", "GJ_button_02.png", [layer, commitId, commitMsg](CCMenuItemSpriteExtra*) {
            ...
            auto revertBtn = makeBtn(
                "Revert", "GJ_button_06.png", [layer, commitId, commitMsg](CCMenuItemSpriteExtra*) {
                    if (layer && !layer->m_listState.closing) {
