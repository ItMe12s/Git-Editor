...
namespace git_editor {
    ...
    CommitMessageLayer* CommitMessageLayer::create(
        ConfirmFn onConfirm, std::string title, std::string buttonLabel, std::string initialText,
        CloseFn onClose
    ) {
    ...
    bool CommitMessageLayer::init(
        ...
        auto confirmSpr =
            ButtonSprite::create(buttonLabel.c_str(), "bigFont.fnt", "GJ_button_01.png", .8f);
        confirmSpr->setScale(.7f);
        auto confirmBtn = CCMenuItemSpriteExtra::create(
            confirmSpr, this, menu_selector(CommitMessageLayer::onConfirmClicked)
        );
