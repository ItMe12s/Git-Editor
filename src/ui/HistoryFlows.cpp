...
namespace git_editor {
    ...
    namespace {
        ...
        std::vector<CommitId> selectedOldestFirst(
            std::vector<Row> const& commitsDesc, std::set<CommitId> const& selected
        ) {
    ...
    void HistoryLayer::onSquashPressed() {
        ...
        for (auto id : idsOldestFirst) {
            auto it = std::find_if(commits.begin(), commits.end(), [id](CommitSummary const& row) {
                return row.id == id;
            });
    ...
    void HistoryLayer::onSquashConfirmed(std::vector<CommitId> idsOldestFirst, std::string defaultMsg) {
        ...
        auto popup = CommitMessageLayer::create(
            ...
            defaultMsg,
            [self]() {
                if (self && !self->m_listState.closing) finishBusyAction(self->m_busy);
            }
        );
        if (popup) popup->show();
            ...
            prepared_editor_flow::OutcomeHandlers{
                ...
                .onPrepareError =
                    [](std::string const& error) {
                        Notification::create(("Squash failed: " + error).c_str(), NotificationIcon::Error)
                            ->show();
                    },
                .onFinalizeError =
                    [](std::string const& error) {
    ...
    void HistoryLayer::startCheckoutFlow(CommitId commitId, std::string const& commitMsg) {
        ...
        createQuickPopup(
            ...
            ("Load state of commit \"" + shorten(commitMsg, 40) +
             "\"? A new auto-revert commit will be added on top of HEAD.")
                .c_str(),
            "Cancel",
            "Checkout",
        ...
        createQuickPopup(
            ...
            [self, editorRef, pauseRef, levelKey, commitId](FLAlertLayer*, bool yes) {
                ...
                prepared_editor_flow::run<RevertPayload, PendingHeadUpdate, CommitId>(
                    ...
                    [](Prepared<RevertPayload> const& prep) {
                        return prep.pendingHead;
                    },
