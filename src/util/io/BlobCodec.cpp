...
namespace git_editor {
    ...
    std::string blobFootprintLimitMessage(std::uint64_t rawSize) {
        return "delta payload size " + std::to_string(rawSize) + " exceeds blob footprint cap " +
            std::to_string(kMaxBlobFootprintBytes);
    }
    ...
    std::optional<std::string> compressBlob(std::string const& raw, int level) {
        ...
        int rc = compress2(
            ...
            &dstLen,
            reinterpret_cast<Bytef const*>(raw.data()),
    ...
    std::optional<std::string> decompressBlob(std::string const& bytes) {
        ...
        if (n > kMaxBlobFootprintBytes) {
            geode::log::error(
                "decompressBlob: uncompressed size {} exceeds cap {}", n, kMaxBlobFootprintBytes
            );
            return std::nullopt;
        }
        std::string out(n, '\0');
