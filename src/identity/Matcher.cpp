...
namespace git_editor {
    ...
    namespace {
        ...
        SpatialCell spatialCellOf(FieldMap const& f) {
            ...
            cell.cx =
                static_cast<int>(std::floor(parseDoubleOr(f, key::kX, 0.0) / kSpatialThreshold));
            cell.cy =
                static_cast<int>(std::floor(parseDoubleOr(f, key::kY, 0.0) / kSpatialThreshold));
            return cell;
        }
    ...
    void assignUuids(LevelState const& previous, LevelState& incoming, AssignUuidStats* stats) {
...

        {
            ...
            for (auto const& [u, _] : previous.objects)
                orderedPrev.push_back(u);
        ...
        for (auto const& [u, _] : incoming.objects)
            orderedIncoming.push_back(u);
